...
var CAS = (module.exports = function CAS(options) {
  ...
  if (!cas_url.hostname) {
    throw new Error(
      'Option `base_url` must be a valid url like: https://example.com/cas'
    );
  }
  ...
  if (options.external_pgt_url) {
    ...
    if (!pgt_url.hostname) {
      throw new Error(
        'Option `external_pgt_url` must be a valid url like: https://example.com:8989/callback'
      );
    }
    ...
    if (options.external_proxy_url) {
      ...
      if (!proxy_url.hostname) {
        throw new Error(
          'Option `external_proxy_url` must be a vald url like: https://example.com:8080/'
        );
      }
  ...
  // Internal PGT callback server
    ...
    if (!this.ssl_key || !this.ssl_cert) {
      throw new Error(
        'Options `ssl_key` and `ssl_cert` are required because you specified `pgt_server`'
      );
    }
    this.pgt_host = options.pgt_host;
    if (!this.pgt_host) {
      throw new Error(
        'Option `pgt_host` is required because you specified `pgt_server`'
      );
    }
    //// Optional
...
CAS.prototype.logout = function(req, res, returnUrl, doRedirect) {
  ...
  var redirectURL =
    'https://' + this.hostname + ':' + this.port + this.base_path + logout_path;
  res.writeHead(307, { Location: redirectURL });
...
CAS.prototype.validatePromise = function(ticket, service, renew) {
      ...
      function(res) {
        ...
        res.on('end', function() {
          ...
          if (cas_version < 2.0) {
            ...
            if (sections.length >= 1) {
              if (sections[0] == 'no') {
                reolve({ authenticated: false });
              } else if (sections[0] == 'yes' && sections.length >= 2) {
                resolve({ authenticated: true, username: sections[1] });
              }
            }
            // Format was not correct, error
          ...
          // CAS 3.0 (XML response, and extended attributes)
            ...
            if (elemSuccess && elemSuccess.length > 0) {
              ...
              // Look for optional proxy granting ticket
              var pgtIOU;
              var elemPGT = elemSuccess
                .find('cas\\:proxyGrantingTicket')
                .first();
              if (elemPGT) {
                pgtIOU = elemPGT.text();
              }
          ...
          // CAS 2.0 (XML response, and extended attributes)
            ...
            if (elemFailure && elemFailure.length > 0) {
              ...
              var message = 'Validation failed [' + code + ']: ';
              message += elemFailure.text();
              reject(new Error(message), false);
              return;
            }
 ...
 * @api public
 */
CAS.prototype.startPgtServer = function(
  key,
  cert,
  ca,
  callbackHost,
  ...
  if (proxyPort) {
    ...
    proxyServer.addListener('request', function(req, res) {
      ...
      self.proxiedRequest(pgtIOU, targetOptions, function(
        err,
        targetReq,
        targetRes
      ) {
...
CAS.prototype.proxiedRequest = function(pgtIOU, options, callback) {
  if (this.external_proxy_url) {
    this.proxiedRequestExternal(
      this.external_proxy_url,
      pgtIOU,
      options,
      callback
    );
 ...
 * @api public
 */
CAS.prototype.proxiedRequestExternal = function(
  proxyURL,
  pgtIOU,
  options,
  callback
